...
const Browse = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/v1/users/public`)
      .then((res) => {
        let usernames = []
        res.data.data.map((singles)=>{
          for(let i=1;i<=singles.recipes.length;i++) usernames = [...usernames , singles.username]
        })

        let dumm = []
        res.data.data
          .map((recipie)=>{
          return recipie.recipes
          })
          .map((dums)=>{
          dumm = [...dumm, ...dums]
          }) 
          let exe = []
          dumm.forEach((ele,i,arr) => {
            exe = [...exe, {...ele, ...{ username : usernames[i]}}]
          });
